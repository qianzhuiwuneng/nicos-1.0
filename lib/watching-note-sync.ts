...
import type { WatchingJourneyFilm } from "@/lib/watching-journey";
import { toPlainText } from "@/lib/weekly-richtext";

export type SyncedWatchingNote = {
  sectionTitle: string;
  excerpt: string;
  fullText: string;
  sourceWeek: number;
...
function pickPromptForFilm(film: WatchingJourneyFilm, prompts: WeeklyReviewPrompt[]): WeeklyReviewPrompt | undefined {
  const exact = prompts.find((p) => p.id === film.reflectionPromptId);
  if (exact) return exact;
  const normalizedTitle = normalizeLabel(film.title);
  return prompts.find((p) => normalizeLabel(p.label).includes(normalizedTitle));
}

export async function buildSyncedWatchingNotes(
  films: WatchingJourneyFilm[]
): Promise<Record<string, SyncedWatchingNote>> {
  const weeks = Array.from(new Set(films.map((f) => f.reflectionWeek ?? f.week)));
  ...
  const out: Record<string, SyncedWatchingNote> = {};
  for (const film of films) {
    const sourceWeek = film.reflectionWeek ?? film.week;
    const prompts = weekPrompts.get(sourceWeek) ?? [];
    const values = weekValues.get(sourceWeek) ?? {};
    const prompt = pickPromptForFilm(film, prompts);
    const promptId = prompt?.id ?? film.reflectionPromptId;
    if (!promptId) continue;
    const fullText = (values[promptId] ?? "").trim();
    if (!fullText) continue;
    out[film.id] = {
      sectionTitle: prompt?.label ?? `${film.title} 电影观后感`,
